...
class AuthsTable:
    ...
    def insert_new_auth(
        ...
        with get_db() as db:
            ...
            log.info(f"insert_new_auth cell_phone: {cell_phone}")
    ...
    def authenticate_user(self, email: str, password: str) -> Optional[UserModel]:
        log.info(f"authenticate_user: {email}, password: {password}")
        try:
            with get_db() as db:

                auth = db.query(Auth).filter_by(email=email, active=True).first()
                if auth:
                    log.info(f"auth: {auth.password}")
    ...
    def authenticate_user_by_cell_phone(self, cell_phone: str, password: str) -> Optional[UserModel]:
        log.info(f"authenticate_user_by_cell_phone: {cell_phone}, password: {password}")
        try:
            with get_db() as db:

                auth = db.query(Auth).filter_by(cell_phone=cell_phone, active=True).first()
                if auth:
                    log.info(f"auth new: {auth.password}")
                    if verify_password(password, auth.password):
                        user = Users.get_user_by_id(auth.id)
                        return user
                    else:
